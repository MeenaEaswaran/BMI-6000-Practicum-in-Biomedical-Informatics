...
objs_tumor<- readRDS("/Users/meena/Desktop/Practicum Fall 2025/Approach_2_case_control_integratedspace/9_cellchat_seurat_Tumor_SCT.rds")

DefaultAssay(objs_tumor) #check if SCT

# Extract the normalized data
data_input <- GetAssayData(objs_tumor, assay = "SCT", layer = "data")

#Create CellChat object
cellchat <- createCellChat(object = data_input, meta = objs_tumor@meta.data, group.by = "SingleR_ENCODE") #we are grouping by cell types similar to the original paper
...
ggsave("9_cellchatDB_human_database_tumorsamples.jpg", units="in", width=10, height=8, dpi=300)
...
netVisual_circle(cellchat@net$count, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Number of interactions:Tumor samples")
netVisual_circle(cellchat@net$weight, vertex.weight = groupSize, weight.scale = T, label.edge= F, title.name = "Interaction weights/strength:Tumor samples")
...
pathways.show.all <- cellchat@netP$pathways
pathways.show.all #66 pathways here
...
#do below to avoid python UMAP error
library(reticulate)
...
library(future)
options(future.globals.maxSize = 8 * 1024^3)   # 8 GB; bump if needed
...
library(future) #need to increase the size due to error
#error Caught FutureError. Canceling all iterations ...
#Error: Will not launch future due to the size of the globals 1.25 GiB exceeds 0.98 GiB. The total size of the 8 globals exported for future expression (‘{; "# future::getGlobalsAndPackages(): wrapping the original future"; "# expression in do.call(), because function called uses '...'  "; "# as a global variable                                         "; do.call(function(...) {; "# future::getGlobalsAndPackages(): FUN() uses '...' internally "; ...; }; }, args = future.call.arguments); }’) is 639.85 MiB The three largest globals are ‘...future.FUN’ (639.85 MiB of class ‘function’), ‘data.use’ (2.01 KiB of class ‘numeric’) and ‘kRange’ (103 bytes of class ‘numeric’)
...
netVisual_embedding(cellchat, type = "functional", label.size = 3.5) #SAVE
netVisual_embeddingZoomIn(cellchat, type = "functional", nCol = 2) #SAVE
...
#> Classification learning of the signaling networks for a single dataset
# Visualization in 2D-space
netVisual_embedding(cellchat, type = "structural", label.size = 3.5) #SAVE

netVisual_embeddingZoomIn(cellchat, type = "structural", nCol = 2) #SAVE


#Save the CellChat object
saveRDS(cellchat, file = "9_cellchatouput_tumorsamplesonly.rds")
