...
#Re-run LDA/PCA/UMAPs on the RPCA integrated space
#Should lready have 'integrated.dr' from RPCA Integration
Reductions(objs)    # should include "integrated.dr"

# Rebuild graph + clusters + UMAP from the RPCA-integrated reduction
...
# UMAP colored by clusters / by sample (uses the RPCA UMAP)
p_clusters <- DimPlot(
  objs, reduction = "umap.RPCA",
  group.by = "seurat_clusters", label = TRUE, repel = TRUE
) + ggtitle("Clusters (RPCA integrated)")
...
# Save image
ggsave(
  filename = "5_RPCAintegratedUMAP_coloredbyClusters_originalsamples.jpg",
...
) + ggtitle("RPCA integrated: clusters split by condition")
p_split

# Save image
ggsave(
  filename = "5_RPCAintegratedUMAP_coloredbycondition_acrossallsamples.jpg",
  units = "in", width = 12, height = 8, dpi = 300
)

# color by Sample on the same RPCA UMAP
DimPlot(objs, reduction = "umap.RPCA", group.by = "Sample") +
  ggtitle("UMAP colored by Sample (RPCA integrated)")

# Save integrated seurat object
saveRDS(objs, "/Users/meena/Desktop/Practicum Fall 2025/Approach_2_case_control_integratedspace/5_RPCA_integrated_seurat_30dims.rds")
