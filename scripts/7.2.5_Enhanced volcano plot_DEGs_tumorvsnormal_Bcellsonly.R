...
#import data
library(readr)
b <- read.csv("/Users/meena/Desktop/Practicum Fall 2025/Approach_2_case_control_integratedspace/imagesORcsv/7_Differentialexpressionforspecificcelltypes_Normalvscontrol/RAW_unfiltered_DE_Tumor_vs_Normal_allBcells_SCT.csv")
head(b)
library(ggplot2)
...
ggsave("7_Bcells_volcanoplot_defaultcolors_TvsN.jpg", units="in", width=10, height=8, dpi=300)
...
#Modify colors
keyvals <- ifelse(
  b$avg_log2FC < -0.58 & b$p_val_adj <= 0.05 ,'#0072b2', 
  ifelse(b$avg_log2FC > 0.58 & b$p_val_adj <= 0.05 , '#DC3220', '#D6D1D1')
)
...
# Calculate counts
upregulated_count <- sum(b$avg_log2FC > 0.58 & b$p_val_adj <= 0.05, na.rm = TRUE)
downregulated_count <- sum(b$avg_log2FC < -0.58 & b$p_val_adj <= 0.05, na.rm = TRUE)
...
EnhancedVolcano(b,
                ...
                y = 'p_val_adj',
                title = 'Tumor vs. normal - B-cells',
                selectLab = rownames(b)[which(names(keyvals) %in% c('Upregulated', 'Downregulated'))],
                xlab = expression(bold(log[2] ~ fold ~ change)),
                ylab = expression (bold(''-''~log[10] ~ padj)),
                pCutoff = 5.00E-02,
                FCcutoff = 0.58,
  ...
  annotate("text", x = 4, y = 75, label = paste0("#Downregulated genes: ", downregulated_count),
           hjust = 0, size = 4, color = "#0072b2") +
  # Annotate upregulated count on the right side top
  annotate("text", x = 4, y =80, label = paste0("#Upregulated genes: ", upregulated_count),
           hjust = 0, size = 4, color = "#DC3220")

ggsave("7_Bcells_volcanoplot_finalcolors_TvsN.jpg", units="in", width=10, height=8, dpi=300)
...
additional_genes <- c("CD79B")   
#"CD19" and CD79A not differential expressed. Not shown.

# Check if the additional genes are in the dataset
valid_additional_genes <- additional_genes[additional_genes %in% b$gene]
# Combine the additional genes with the existing selected labels
select_labels <- unique(c(rownames(b)[which(names(keyvals) %in% c('Upregulated', 'Downregulated'))], valid_additional_genes))
...
#TOsave images without changing up and down colors
ggsave("7_Bcells_volcanoplot_finalcolors_TvsN_allcellsannotated.jpg", units="in", width=10, height=8, dpi=300)
########################################################################################
